...
public partial class MainWindow
{
    ...
    private QueueItem? SelectNextQueueItemForContinuousLoop(bool preview = false)
    {
        ...
        // work is appended below because it is explicit user work and does not depend on automation toggles.
    ...
    // Persistent Queue-page items are explicit user work and must run even when that village's
    // automation toggle/group is off. Those settings only gate runtime items generated by the loop.
    private bool IsQueueItemAllowedByAutomationSettings(QueueItem item)
    {
        return !item.IsRuntimeOnly
            || (IsQueueItemVillageEnabled(item) && IsQueueItemGroupEnabledForItsVillage(item));
    }
