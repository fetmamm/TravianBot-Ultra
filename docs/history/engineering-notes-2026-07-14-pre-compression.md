...
- Proxyfel (`ERR_*PROXY*`/`ERR_SOCKS_CONNECTION_FAILED`) och Chromiums natverksfelsida ar unavailable,
  ...
  wake-signaler och keep-alive far inte avbryta den. Under defer-fonstret skippar background resource/inbox reads.
  Proxyn far bara bypassas av recovery nar `Never use own IP` ar av. Saknat `Load`-event far bara godtas nar HTTP-URL, icke-neterror DOM och Travian-shell
  kan bekraftas.
...
- `config/buildings_catalog.json` ar versionsstyrd Official-data och embed:as i Worker-assemblyn som
  `TbotUltra.Worker.Data.buildings_catalog.json`; runtime far inte lasa den fran anvandarens `config/`.
  Alla byggnader ska ha sammanhangande nivaer `1..max_level`; laddfel loggas en gang vid startup och
  stanger av estimat utan per-task-alarmspam. Tribe-matrisen omfattar Romans, Teutons, Gauls, Egyptians,
  Huns och Spartans (inte Vikings) och ska verifieras mot Travian Knowledge Base.
- Klassificerad queue-full-defer behaller Worker-lasningens `NextAttemptAt`; Desktop far inte ateraktivera
  ...
  Stateless timer-/delay-beslut for full ko ligger i `ConstructionHumanizeCalculator`; browser-, session- och
  payload-state ligger kvar i `TravianClient`.
...
- 2026-07-14: Diagnostics export uses a busy overlay while the archive is created.
- 2026-07-14: Sanitize mutable JSON arrays from a snapshot before replacing elements.
